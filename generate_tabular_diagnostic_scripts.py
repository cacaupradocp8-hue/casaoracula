...
try:
    ...
    # Fallback if the file is missing in this context, though it should be there
    fks = []

TOTAL_FKS = len(fks)
CHUNKS = 8
PER_CHUNK = math.ceil(TOTAL_FKS / CHUNKS) if TOTAL_FKS > 0 else 0

def generate_part(part_num, start_idx, end_idx):
    chunk_fks = fks[start_idx:end_idx]
    
    sql = f"""-- BLOCO 07B - FOREIGN KEYS DIAGNOSTIC TABULAR (PARTE {part_num} de {CHUNKS})
...
if TOTAL_FKS > 0:
    for i in range(CHUNKS):
        ...
        part_num = i + 1
        filename = f"bloco_07b_dry_run_TABULAR_part_{part_num:02d}.sql"
...
with open('bloco_07b_dry_run_TABULAR_INDEX.md', 'w') as f:
    f.write(index_md)

print(f"Generated {CHUNKS} TABULAR parts and index.")
