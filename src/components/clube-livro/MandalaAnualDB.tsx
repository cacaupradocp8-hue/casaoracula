...
// Layout retangular com cards preservado
...
import { BookOpen, X, ChevronRight, Lock, Map, List, Headphones } from 'lucide-react';
...
// ============================================
const LAYERS = {
  MATRIZ: { label: 'Matriz', cor: 'hsl(43, 60%, 54%)', bg: 'hsl(43, 60%, 54%, 0.12)' },
  ESTACAO: { label: 'Estação', cor: 'hsl(212, 50%, 50%)', bg: 'hsl(212, 50%, 36%, 0.12)' },
} as const;
...
function EstacaoCard({
  ...
  return (
    ...
    >
      <Card
        className={cn(
          'cursor-pointer transition-all duration-300 overflow-hidden hover:shadow-lg hover:scale-[1.02]',
          isActive
    ...
    >
      <div className="p-6 space-y-5">
        ...
        <div className="space-y-2">
          <Button variant="outline" className="w-full justify-start gap-2" onClick={() => onNavigate(`/clube-livro/estacao/${estacao.id}`)}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-center gap-1 p-1 bg-muted/50 rounded-lg w-fit mx-auto">
        <Button variant={viewMode === 'map' ? 'default' : 'ghost'} size="sm" onClick={() => setViewMode('map')} className="gap-1.5 text-xs h-8">
          <Map className="w-3.5 h-3.5" /> Mapa
        </Button>
        <Button variant={viewMode === 'list' ? 'default' : 'ghost'} size="sm" onClick={() => setViewMode('list')} className="gap-1.5 text-xs h-8">
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {estacoes.map((est, i) => (
                <EstacaoCard key={est.id} estacao={est} index={i} onClick={() => setSelectedEstacao(est)} />
              ))}
            </div>
          </div>
        </div>
