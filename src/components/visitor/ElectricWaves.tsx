...
/**
 * ElectricWaves — Ondas douradas sutis fluindo pela tela.
 */
export function ElectricWaves({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const waves = [
      { y: 0.3, amplitude: 30, frequency: 0.008, speed: 0.012, opacity: 0.10, width: 1.2 },
      { y: 0.4, amplitude: 22, frequency: 0.012, speed: 0.018, opacity: 0.08, width: 0.8 },
      { y: 0.5, amplitude: 40, frequency: 0.006, speed: 0.008, opacity: 0.12, width: 1.5 },
      { y: 0.6, amplitude: 18, frequency: 0.015, speed: 0.022, opacity: 0.07, width: 0.6 },
      { y: 0.7, amplitude: 35, frequency: 0.009, speed: 0.014, opacity: 0.09, width: 1.0 },
    ];
    ...
    const draw = () => {
      ...
      for (const wave of waves) {
        ...
        ctx.strokeStyle = `rgba(212, 175, 55, ${wave.opacity * 0.3})`;
      ...
      for (let i = 0; i < waves.length - 1; i++) {
        ...
        if (dist < 60) {
          const sparkOpacity = (1 - dist / 60) * 0.3;
          ctx.beginPath();
          ctx.arc(sparkX, midY, 2, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(230, 200, 120, ${sparkOpacity})`;
          ctx.fill();

          ctx.beginPath();
          ctx.arc(sparkX, midY, 8, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(212, 175, 55, ${sparkOpacity * 0.3})`;
          ctx.fill();
        }
      }
