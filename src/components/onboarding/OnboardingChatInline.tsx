...
interface OnboardingChatInlineProps {
  onComplete: () => void;
}
...
const CLOSING_MESSAGE = 'A Voz se recolhe agora. Quando estiver pronta, decida se quer habitar esta Casa.';

const FALLBACK_RESPONSES = [
  'Há mistérios que pedem tempo. Continue explorando a Sala da Visitante.',
  'O véu se adensa... O silêncio também é resposta.',
...
export function OnboardingChatInline({ onComplete }: OnboardingChatInlineProps) {
      ...
      const { data, error } = await supabase.functions.invoke('ai-chat', {
        body: {
          ...
          context: {
            ...
            agentId: '2fe0dad4-af99-43d1-91cc-90dd1059ed2f',
          },
        },
      });
  ...
  }, [input, isLoading, isEnded, messages, getFallbackResponse]);
  ...
  return (
    <div className="flex flex-col h-[400px]">
      ...
      <div className="p-4 border-b border-border/30 flex items-center gap-2">
        ...
        <span className="text-sm text-purple-400 font-medium">Voz Revelada</span>
        <span className="text-xs text-muted-foreground ml-auto">Modo Visitante</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {messages.map((message, index) => (
            ...
            >
              <div
                ...
                `}
              >
                <p className={`leading-relaxed ${message.role === 'assistant' ? 'italic' : ''}`}>
                  {message.content}
                </p>
                {message.isError && (
        ...
        {isLoading && (
          ...
          >
            <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl rounded-bl-md px-4 py-2.5">
      ...
      <div className="p-4 border-t border-border/30">
        {isEnded ? (
          ...
          >
            <Button variant="gold" onClick={onComplete} size="sm" className="w-full">
              Guardar esta conversa
            </Button>
          </motion.div>
        ) : (
