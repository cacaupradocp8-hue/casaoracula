...
export default function Auth() {
  ...
  // ─── Main auth view ───
  return (
    <div className="min-h-[100dvh] relative flex flex-col items-center justify-center p-4">
      <ImmersiveBg />
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        >
          ...
          <Link to="/tour" className="text-xs text-muted-foreground/60 hover:text-gold/80 transition-colors">
            Conhecer a Casa →
          </Link>
        </motion.div>
        ...
        >
          <Logo size="lg" variant="vertical" className="justify-center mb-3" />
        ...
        >
          ...
          <div className="relative rounded-2xl bg-card/40 backdrop-blur-xl border border-gold/10 p-5">
            <p className="text-foreground/90 text-sm leading-relaxed font-display tracking-wide text-center">
        ...
        >
          <GlassContainer>
            <Tabs defaultValue="login" className="w-full">
              <TabsList className="grid w-full grid-cols-2 mb-8 bg-background/20 p-1 rounded-xl">
                <TabsTrigger value="login" className="rounded-lg py-2.5 transition-all data-[state=active]:bg-gold data-[state=active]:text-midnight data-[state=active]:shadow-lg font-medium text-sm">Entrar</TabsTrigger>
                <TabsTrigger value="signup" className="rounded-lg py-2.5 transition-all data-[state=active]:bg-gold data-[state=active]:text-midnight data-[state=active]:shadow-lg font-medium text-sm">Criar Conta</TabsTrigger>
              </TabsList>
                ...
                >
                  ...
                  <motion.div variants={{ hidden: { y: 10, opacity: 0 }, visible: { y: 0, opacity: 1 } }} className="space-y-2.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="flex h-12 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-gold/30 transition-all"
                      />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/60 hover:text-gold transition-colors">
                  ...
                  <motion.div variants={{ hidden: { y: 10, opacity: 0 }, visible: { y: 0, opacity: 1 } }}>
                    <Button type="submit" variant="gold" className="w-full h-12 text-sm font-semibold rounded-xl shadow-premium-glow hover:scale-[1.02] active:scale-[0.98] transition-all" disabled={isLoading}>
                      {isLoading ? 'Conectando...' : getCopyByKey('btn_atravessar_limiar', 'Atravessar o limiar')}
                    </Button>
                  </motion.div>

                  <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="relative my-4">
                    <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-white/5" /></div>
                    <div className="relative flex justify-center text-[10px] uppercase tracking-[0.2em] font-medium"><span className="bg-card/0 px-4 text-muted-foreground/40">ou</span></div>
                  </motion.div>

                  <motion.div variants={{ hidden: { y: 10, opacity: 0 }, visible: { y: 0, opacity: 1 } }}>
                    <Button type="button" variant="outline" className="w-full h-12 flex items-center justify-center gap-3 bg-white/5 border-white/10 hover:bg-white/10 hover:border-gold/30 rounded-xl transition-all" onClick={handleGoogleSignIn} disabled={googleLoading}>
                  ...
                  <motion.button 
                    ...
                    className="w-full text-xs text-muted-foreground/60 hover:text-gold transition-colors text-center pt-2"
                  >
              ...
              <TabsContent value="signup">
                <form onSubmit={handleSignup} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="signup-name" className="text-foreground/80 text-sm">Nome</Label>
                    <input
                      id="signup-name"
                      type="text"
                      ...
                      required
                      className="flex h-11 w-full rounded-lg border border-border/40 bg-background/50 px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-gold/40 disabled:cursor-not-allowed disabled:opacity-50"
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <Button type="submit" variant="gold" className="w-full h-11 text-sm shadow-[0_0_30px_-6px_hsl(var(--gold)/0.3)]" disabled={isLoading}>
                  ...
                  <Button type="button" variant="outline" className="w-full h-11 flex items-center gap-3 bg-background/30 border-border/30 hover:bg-background/50 hover:border-gold/20 text-sm" onClick={handleGoogleSignIn} disabled={googleLoading}>
                    {googleLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : (
                  ...
                  <p className="text-[11px] text-muted-foreground/60 text-center pt-1 leading-relaxed">
                    Ao criar conta, você inicia como Visitante.
                  </p>
                </form>
              </TabsContent>
            </Tabs>
