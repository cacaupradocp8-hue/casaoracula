        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-light tracking-tight text-white/90">
              Sala de <span className="font-semibold text-white">Treinamento</span>
            </h1>
            <p className="text-muted-foreground/60 text-sm tracking-wide uppercase font-medium">
              Onde a teoria se encontra com a prática e a excelência clínica.
            </p>
          </div>
          ...
          <div className="flex items-center gap-6 bg-white/[0.03] border border-white/[0.05] rounded-2xl px-6 py-4 backdrop-blur-sm">
            <div className="flex flex-col items-center gap-1">
              <div className="flex items-center gap-2 text-[#D4AF37]">
                ...
                <span className="text-sm font-bold tracking-widest">{isFormacao ? "EXPERT" : "INICIANTE"}</span>
              </div>
              <Progress value={isFormacao ? 75 : 20} className="w-24 h-1 bg-white/10" />
            </div>
          ...
          <TabsContent value="simulador" className="space-y-10 animate-in fade-in duration-500">
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="text-[9px] border-primary/20 text-primary/60">Foco em Escuta</Badge>
              </div>
            ...
            <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <MiniCard icon={BookOpen} title="Caso Clínico" description="Complexo" isLocked={!isFormacao} onClick={() => {}} />
              <MiniCard icon={Search} title="Leitura de Campo" description="Simbólico" isLocked={!isFormacao} onClick={() => {}} />
              <MiniCard icon={AlertCircle} title="Erro Oculto" description="Ache a falha" isLocked={!isFormacao} onClick={() => {}} />
            </section>
...
function MiniCard({ icon: Icon, title, description, isLocked, onClick }: { icon: any, title: string, description: string, isLocked?: boolean, onClick: () => void }) {
  ...
  return (
    <div 
      onClick={() => isLocked ? navigate('/formacao') : onClick()}
      className={cn(
