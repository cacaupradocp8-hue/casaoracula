...
# Hardcoded existing tables from the manual observation (the list was quite long)
# I will use the list from the tool output I saw earlier.
existing_tables_str = """
...
# We look for CREATE TABLE, then the body, ending with a semicolon on a new line or end of string.
# The schema file usually has ); followed by a newline.
matches = re.finditer(r'CREATE TABLE\s+(?:public\.)?(\w+)\s*\((.*?)\);', schema_sql, re.DOTALL | re.IGNORECASE)
for match in matches:
    table_name = match.group(1).lower()
    full_stmt = match.group(0)
    table_definitions[table_name] = full_stmt
