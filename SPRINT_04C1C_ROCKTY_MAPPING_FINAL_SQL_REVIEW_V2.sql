-- =====================================================================
-- SPRINT_04C1C_ROCKTY_MAPPING_FINAL_SQL_REVIEW_V2.sql
-- Versão: V3.2 FINAL V2 (REVISÃO — NÃO EXECUTAR)
-- Data: 2026-05-12
-- Objetivo: Mapeamento oficial Rockty offer_id → plan interno + portal.
--
-- Correções V2:
--   1. formacao_oracula mapeia para portal 'aluna' (não 'aluna_formacao').
--   2. process_webhook_subscription atualizado para usar ON CONFLICT (user_id, provider, plan_id).
--   3. Sincronização de portal (profiles/user_roles) COMENTADA.
--   4. apply_pending_matricula usa COALESCE para fallback de offer_id.
--   5. Schema real de public.plans confirmado: portal_resultante (não portal_destino).
-- =====================================================================
...
-- =====================================================================
-- ETAPA 1 — Criar plans ausentes (Schema Real: portal_resultante)
-- =====================================================================
...
-- Inserir matriz oficial (formacao_oracula → aluna)
...
-- =====================================================================
-- ETAPA 3 — Estender matriculas_pendentes (colunas ausentes)
-- =====================================================================
...
-- 5.1 apply_pending_matricula com Fallback (COALESCE)
...
BEGIN
  ...
  -- Fallback seguro para capturar o offer_id
  target_offer_id := COALESCE(pending.rockty_offer_id, pending.curso_id, pending.produto_rockty);
  ...
  -- Criar matrícula com plan interno
  ...
  -- Criar/atualizar subscription com plan_id interno (USA NOVA UNIQUE)
  INSERT INTO public.subscriptions
    (user_id, provider, plan_id, status, external_subscription_id, last_event_at)
  VALUES
    (NEW.id, 'rockty', mapping.internal_plan_id, 'active', NULL, now())
  ON CONFLICT (user_id, provider, plan_id) DO UPDATE
    SET status = 'active', last_event_at = now(), updated_at = now();
...
-- 5.2 process_webhook_subscription (Sincronizado com novas UNIQUEs)
...
DECLARE
  ...
  _result jsonb;
BEGIN
  -- Upsert usando a nova UNIQUE composta (user_id, provider, plan_id)
...
-- =====================================================================
-- ETAPA 6 — (COMENTADA) Sincronização de Portais
-- =====================================================================
/*
...
-- =====================================================================
-- ETAPA 7 — Validações (Somente Leitura)
-- =====================================================================
...
BEGIN
  ...
  SELECT count(*) INTO c_test_unmapped FROM public.matriculas_pendentes WHERE rockty_offer_id = 'TEST_UNKNOWN_OFFER' AND processing_status = 'unmapped';
